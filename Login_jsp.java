...
//Database connection string
  ...
  String dLookUp(java.sql.Statement stat, String table, String fName, String where) {
    ...
    try {
	  ...
	  stat1 = conn1.prepareStatement(sql);
	  stat1.setString(1, fNamefName);
	  stat1.setString(2, table);
	  stat1.setString(3, where);
	  
      java.sql.ResultSet rsLookUp = openrs(stat1);
      if (! rsLookUp.next()) {
        rsLookUp.close();
        stat1.close();
        conn1.close();
...
static final String sFileName = "Login.jsp";
  ...
  String LoginAction(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response, javax.servlet.http.HttpSession session, javax.servlet.jsp.JspWriter out, String sAction, String sForm, java.sql.Connection conn) throws java.io.IOException {
    String sLoginErr = "";
    try {
      ...
      switch (iAction) {
        case iloginAction: {
		  ...
		  javax.crypto.SealedObject sPassword = getParam( request, "Password");
          java.sql.ResultSet rs = null;
		  
		  String sql = "select member_id, member_level from members where member_login=? and member_password=?";
...
if ( sForm.equals("Login") ) {
  sLoginErr = LoginAction(request, response, session, out, sAction, sForm, conn);
